...
export default function About() {
    ...
    const [btnw,setBtnw] =useState("Enable Dark Mode")
    const toggleStyle = ()=>{
        if(myStyle.color === 'black'){
            ...
            setBtnw("Disable Dark Mode")
        }else{
            setMyStyle({
                color:'black',
                background:'white',
            })
            setBtnw("Enable Dark Mode")
        }
    }
  ...
  return (
    <div className='container my-2' style={myStyle}>
        ...
        <button onClick={toggleStyle} type="button" className="btn btn-info my-2">{btnw}</button>
    </div>
  )
}
